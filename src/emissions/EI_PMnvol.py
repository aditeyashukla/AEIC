...
def PMnvol_MEEM(
    ...
    if np.min(EI_mass_mode) < 0:
        ...
        kslm         = np.log((3.219 * CI_mass * (1 + bypass_ratio) * 1e3 + 312.5) /
    ...
    pressure_coef    = np.where(alt_rate > 0,
    ...
    Pt_amb = Pamb_cruise * (1 + (kappa - 1) / 2 * machFlight ** 2) ** (kappa / (kappa - 1))
    ...
    def build_interp(arr_mode, Tmax, Tmax_thrust):
        if np.isnan(Tmax_thrust) or Tmax_thrust < 0:
            ...
            if np.isclose(Tmax_thrust, 0.575):
                arr   = np.concatenate(([arr_mode[0]],
                                         arr_mode[0:2], [Tmax], arr_mode[2:], [arr_mode[-1]]))
                tgrid = np.array([-10, 0.07, 0.3, 0.575, 0.85, 1.0, 100])
            elif np.isclose(Tmax_thrust, 0.925):
                arr   = np.concatenate(([arr_mode[0]],
                                         arr_mode[0:3], [Tmax], arr_mode[3:], [arr_mode[-1]]))
                tgrid = np.array([-10, 0.07, 0.3, 0.85, 0.925, 1.0, 100])
            else:
                raise ValueError("Unrecognised *_max_thrust value.")
        return tgrid, arr

    t_mass, arr_mass = build_interp(EI_mass_mode,
                                    EDB_data["EImass_max"],
                                    EDB_data["EImass_max_thrust"])
    t_num,  arr_num  = build_interp(EI_num_mode,
                                    EDB_data["EInum_max"],
                                    EDB_data["EInum_max_thrust"])
