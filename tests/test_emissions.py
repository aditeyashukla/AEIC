...
def test_emission_indices_positive():
    ...
    for field in em.emission_indices.dtype.names:
        assert np.all(em.emission_indices[field] >= 0), f"Negative emission index found for {field}"

def test_emission_indices_finite():
    """Test that all emission indices are finite (no NaN or inf)"""
    for field in em.emission_indices.dtype.names:
        assert np.all(np.isfinite(em.emission_indices[field])), f"Non-finite emission index found for {field}"
    ...
    for field in em.summed_emission_g.dtype.names:
        ...
        if field == 'CO2':
            ...
            original_co2 = em.summed_emission_g[field] - (
                ...
                (np.sum(em.pointwise_emissions_g[field]) + np.sum(em.LTO_emissions_g[field]) + 
                 em.APU_emissions_g[field] + em.GSE_emissions_g[field])
            )
            assert pytest.approx(original_co2, rel=1e-6) == calculated_sum
        else:
            assert pytest.approx(em.summed_emission_g[field], rel=1e-6) == calculated_sum, \
                f"Sum mismatch for {field}: {em.summed_emission_g[field]} vs {calculated_sum}"
...
def test_fuel_burn_decreasing_mass():
    ...
    assert np.all(np.diff(fuel_mass) <= 0), "Fuel mass should be monotonically decreasing"

def test_nox_speciation_conservation():
    """Test that NOx speciation sums to total NOx for LTO"""
    if hasattr(em, 'LTO_noProp') and hasattr(em, 'LTO_no2Prop') and hasattr(em, 'LTO_honoProp'):
        total_prop = em.LTO_noProp + em.LTO_no2Prop + em.LTO_honoProp
        assert pytest.approx(total_prop, abs=1e-3) == 1.0, "NOx speciation fractions should sum to 1"

def test_sox_speciation():
    """Test that SO2 and SO4 are both present and reasonable"""
    so2_total = np.sum(em.pointwise_emissions_g['SO2']) + np.sum(em.LTO_emissions_g['SO2'])
    so4_total = np.sum(em.pointwise_emissions_g['SO4']) + np.sum(em.LTO_emissions_g['SO4'])
...
def test_gse_wnsf_mapping():
    ...
    for wnsf in wnsf_codes:
        ...
        assert temp_em.GSE_emissions_g['CO2'] > 0, f"GSE CO2 not assigned for WNSF {wnsf}"
        assert temp_em.GSE_emissions_g['NOx'] > 0, f"GSE NOx not assigned for WNSF {wnsf}"
...
def test_pm_components_reasonable():
    ...
    pmvol_total = np.sum(em.pointwise_emissions_g['PMvol']) + np.sum(em.LTO_emissions_g['PMvol'])
    pmnvol_total = np.sum(em.pointwise_emissions_g['PMnvol']) + np.sum(em.LTO_emissions_g['PMnvol'])
    
    # Both components should be positive if PM is present
    if pmvol_total > 0 or pmnvol_total > 0:
        assert pmvol_total >= 0 and pmnvol_total >= 0, "Both PM components should be non-negative"
...
def test_trajectory_vs_lto_mode_consistency():
    ...
    if em.traj_emissions_all:
        ...
        assert em.LTO_emissions_g.shape == (), "Should have scalar LTO arrays when traj_emissions_all=True"
    else:
        # Full LTO cycle
        assert len(em.LTO_emission_indices['NOx']) == 4, "Should have 4 LTO modes when traj_emissions_all=False"
...
def test_lifecycle_co2_adjustment():
    ...
    base_co2 = (np.sum(em.pointwise_emissions_g['CO2']) + 
                np.sum(em.LTO_emissions_g['CO2']) + 
                em.APU_emissions_g['CO2'] + 
                em.GSE_emissions_g['CO2'])
    
    assert em.summed_emission_g['CO2'] != base_co2, "Lifecycle CO2 adjustment should modify total"
...
def test_emission_units_consistency():
    ...
    # This is more of a documentation test - ensure the class produces results in expected units
    ...
    if fuel_consumed_kg > 0:
        ...
        assert 2.5 <= co2_per_fuel <= 4.4, f"CO2 per fuel ratio {co2_per_fuel} outside expected range"
...
def test_array_shapes_consistency():
    ...
    assert em.emission_indices['CO2'].shape == (n_traj,), "Emission indices shape mismatch"
    assert em.pointwise_emissions_g['CO2'].shape == (n_traj,), "Pointwise emissions shape mismatch"
    assert em.fuel_burn_per_segment.shape == (n_traj,), "Fuel burn array shape mismatch"
    
    # Summary arrays should be scalars
    assert em.summed_emission_g['CO2'].shape == (1,), "Summed emissions should be scalar"